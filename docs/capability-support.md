...
Source package version: **0.1.13**. Latest published release: **v0.1.13**.
...
| `web-manual-proxy-login` | Branch-only | The current stabilization source lets an Owner create, rotate, revoke, and deliver a manual proxy login and its public Proxy CA through the Web management API. It is not in v0.1.13. |
...
| `codex-reset-credit` | Branch-only | The current stabilization source can inspect and explicitly consume one selected banked reset credit through an Owner-only, confirmed, idempotent management operation. It is not in v0.1.13. |
