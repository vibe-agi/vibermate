...
新请求的 `EgressAttempt.errorClass` 不再把所有发送失败压成
`transport_failed`：仅依据 Go 标准库的错误类型，区分域名解析、证书验证、
连接错误和超时；没有可信类型的错误仍记为 `transport_failed`。不会把任意
`err.Error()`、域名、URL 或证书内容写进该字段。Web/App 根据这份同源证据
显示中英文下一步建议；未知阶段明确说未知，不把无代理推断为必然 DNS/TLS 问题。

隔离的原生 Server + 合成上游 + Playwright 验收在关闭本机上游后证实：
Activity 为 `provider_transport_failed`、EgressAttempt 为 `connection_failed`，
Web 中点开失败 Exchange 后能看到对应提示。其他阶段由包裹错误类型的单元
测试及中英文文案测试覆盖，尚无各阶段的真实浏览器流量样本。
失败终态若没有响应，也不再同时显示“等待最终响应”；Playwright 对此作了断言。
用户主动预览并复制的脱敏诊断保留冻结策略修订和尝试阶段/时间；仍不自动
导出正文、凭据、HTTP 头字段、目标路径或查询。诊断包现在从 Runtime 状态读取
Go 构建信息：优先使用发布模块版本，否则使用 VCS revision，并明确标注 modified；
无可信信息时只写 `development`，不能把诊断 schema 版本当作构建版本。
2026-09-07 的旧记录不会追溯改写；
它们缺少原始底层类型，原因仍不能事后确定。自定义 TLS 指纹的本地准备与
无类型握手失败现在分别记录 `transport_profile_failed` 与 `tls_handshake_failed`；
若错误链同时给出 DNS、证书验证、连接或超时类型，仍优先使用更精确类别。
运行时模型发现复用同一分类，不再无条件压成 `transport_failed`。这些类别仍是
有限阶段事实，不包含任意错误文本，也不能把本次补充当作全部传输故障已可定位。
