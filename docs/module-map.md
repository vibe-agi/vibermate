...
| `internal/codexoauth` | Imported Codex ChatGPT OAuth credential bundles, unverified safe-profile projection, proactive refresh decisions, per-credential single-flight and CAS rotation before an Account lease freezes. | Account selection, Runtime User identity, arbitrary OAuth issuers, browser/device login, raw evidence, or authorization from JWT claims. | Import/schema and account-consistency rejection, full/partial token rotation, permanent reconnect state, transient valid-token continuity, concurrent refresh coalescing and secret-free API tests. |
...
browser/device OAuth account connectors, live provider acceptance, automatic account
failover, plugins, QualityRun, Language Bridge, a fresh macOS Keychain
mutation acceptance run, Developer ID signing, notarization, installation, or
