...
| `internal/runtimepersistence` | The single SQLite schema baseline, schema identity, repository operation gate, Environment/draft/assignment, Capture, ProviderAccount configuration, Activity, approval, connection, egress and runtime metadata authority. | Snapshot compilation, protocol behavior, secret values, or HTTP DTOs. | CAS, rollback, ambiguous-commit reconciliation, permissions, corruption, cancellation/drain, source-bound schema, archive recovery, reopen and race tests. |
...
| `internal/acpbridge` | Uncomposed ACP transport foundation: byte-opaque duplex copying, fixed buffers, owned stream teardown, half-close/drain, destination byte counts and payload-free failures. | Public CLI, child supervision/TTY/auth, protocol interpretation, permission decisions, session/workspace authority, HTTP capture/routing, persistence or UI. | Exact-byte/large-frame/vendor-extension fixtures, EOF/partial-delivery/backpressure/short-write/close-failure tests, cancellation/ownership/race checks, and a synthetic real child with separate stderr. See the [staged ACP plan](plans/2026-09-07-acp-integration.md); transport tests do not establish product compatibility. |
