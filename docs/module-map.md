...
| `internal/workspacedefault` | Optional machine-and-workspace-scoped Environment preference for future managed Captures. Explicit launch selection wins; absence resolves to `system_transparent`. | Current Capture mutation, routing, account/model/plugin choice, endpoint authority, or Root admission. | Canonical identity, CAS set/clear, active-user-Environment validation, concurrent writes, SQLite reopen and race tests. |
...
| `internal/exchangecontent` | Optional retention-bound neutral Request/Response, message, tool-call/result, and usage evidence for one frozen Exchange. It sanitizes content before persistence and supports full, metadata-only, and off policies. | Raw wire bodies, headers, credentials, provider extensions, current-configuration reinterpretation, or body-free Activity/connection/egress audit. | Canonical closed JSON, secret/home-path sanitization, metadata projection, alias isolation, expiry/purge, shutdown admission, SQLite reopen and race tests. |
