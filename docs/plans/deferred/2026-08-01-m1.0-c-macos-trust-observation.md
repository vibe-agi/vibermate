...
Status: implemented and extended on 2026-09-03 with foreground Root trust UX
Created: 2026-08-01
Implementation baseline: `329490b859089e6c0321b60c84268c74504e7d7d`

> Historical scope note: the document below preserves the original read-only
> milestone and is no longer the complete implementation authority. The
> shipped source boundary now adds a digest-bound public-certificate handoff,
> explicit foreground installation into the current login's keychain and user
> trust domain through Security.framework, exact-certificate trust observation,
> manual removal guidance, and a restart-bound reset admitted only after the old
> Root is absent. It does not install a privileged helper or mutate system-wide
> trust. Startup, Go sidecars and remote Hosts never mutate user trust. See
> `docs/module-map.md` and the current native Runner, `internal/systemtrust`,
> `internal/desktoptrust`, and `internal/localca` contracts.
