...
audit to a typed client-side HTTP outcome, their Codex Hold detail overstates
client deltas, and their Codex tool detail says `Write` instead of `exec`. Root
`PLAN.md` tracks the narrow v5 evidence correction. No runtime rollback is
required.
...
typed HTTP outcome; M0.9.1 must require both sources and freeze a v5 report.
