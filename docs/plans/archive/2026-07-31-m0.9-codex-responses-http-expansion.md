...
runtime path and artifact provenance, but they do not independently require the
typed Codex fallback event, their Codex Hold detail overstates client deltas,
and their Codex tool detail says `Write` instead of `exec`. Root `PLAN.md`
tracks the narrow v5 evidence correction. No runtime rollback is required.
...
before body read, Exchange, or original egress. The v4 packaged report proves
the two proxy connection timelines but does not independently assert the typed
Codex fallback event; M0.9.1 must require both sources and freeze a v5 report.
