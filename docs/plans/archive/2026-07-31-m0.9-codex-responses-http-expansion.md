...
Status: complete via M0.9.1 corrected evidence
Created: 2026-07-31
...
runtime path and artifact provenance, but their fallback, Codex Hold, and tool
details are superseded by the clean M0.9.1 v5 reports. The corrected evidence
requires a Codex-surfaced HTTP 426, the proxy connection audit, and the exact
Runtime Activity reason; names Codex `exec`; and makes no client-visible delta
claim. No runtime rollback was required.
...
HTTP status plus the exact Runtime Activity reason. M0.9.1 now requires all
three sources and freezes v5 reports.
