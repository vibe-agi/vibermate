...
Status: implementation frozen; evidence correction active
Created: 2026-07-31
Implementation frozen: 2026-07-31
Implementation baseline: `7560937a358f23d1924a7dffa1978537171efb12`

Evidence correction notice: the retained v4 reports prove the underlying
runtime path and artifact provenance, but they do not bind the proxy fallback
audit to both a typed client HTTP status and the exact Runtime Activity reason,
their Codex Hold detail overstates client deltas, and their Codex tool detail
says `Write` instead of `exec`. Root `PLAN.md` tracks the narrow v5 evidence
correction. No runtime rollback is required.
...
before body read, Exchange, or original egress. The fixed client's startup
prewarm treats 426 as an immediate silent transport switch, so a dedicated
fallback warning is not part of this path. The v4 packaged report proves the
two proxy connection timelines but does not bind them to the same client's
typed HTTP status plus the exact Runtime Activity reason; M0.9.1 must require
all three sources and freeze a v5 report.
Semantic acceptance turns use the fixed client's supported explicit HTTP
provider profile so independent processes do not race session-local WebSocket
negotiation. This evidence does not establish successful Responses WebSocket
semantics.
