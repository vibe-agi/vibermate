...
check-flutter-macos: check-flutter
	...
	VIBERMATE_LIVE_TEST_APP="$(CURDIR)/dist/ViberMate.app" go test -count=1 -run '^TestPackaged(FlutterDesktopShell|ACPThroughDesktopApp)Live$$' ./cmd/vibermate-acceptance
	cd ui/flutter_app && VIBERMATE_LIVE_TEST_DAEMON="$(CURDIR)/dist/ViberMate.app/Contents/MacOS/vibermated" VIBERMATE_LIVE_TEST_COMMAND="$(CURDIR)/dist/ViberMate.app/Contents/MacOS/vibermate" flutter test --concurrency=1 test/live_runtime_test.dart test/acp_live_runtime_test.dart
