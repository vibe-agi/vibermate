...
func (gate *lifecycleGate) begin(caller context.Context) (context.Context, func(), error) {
	if caller == nil {
		return nil, nil, fmt.Errorf("%w: context is nil", ErrInvalidCommand)
	}
	...
	stop := context.AfterFunc(owner, func() { cancel(ErrRuntimeStopping) })
	var once sync.Once
	finish := func() {
		once.Do(func() {
			stop()
			cancel(errors.New("ProxyClient operation finished"))
			gate.finish()
		})
	}
...
func (gate *lifecycleGate) finish() {
	...
	gate.active--
	if gate.active < 0 {
		panic("ProxyClient lifecycle active operation count became negative")
	}
...
func (gate *lifecycleGate) drain(ctx context.Context) error {
	if ctx == nil {
		return fmt.Errorf("%w: drain context is nil", ErrInvalidCommand)
	}
	gate.mu.Lock()
	if !gate.accepting && gate.active == 0 {
		gate.mu.Unlock()
	...
	case <-drained:
		return nil
	case <-ctx.Done():
		return fmt.Errorf("drain ProxyClient operations: %w", ctx.Err())
	}
}
