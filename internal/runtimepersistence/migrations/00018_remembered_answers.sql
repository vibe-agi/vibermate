-- +goose Up
-- An answer can now be remembered. Remembering writes a connection rule, and
-- design 06's INV-APPROVAL-TERMINAL-ATOMIC puts that write in the same commit
-- as the decision: a decision that survived without its rule would ask again,
-- and a rule that survived without its decision would answer a question nobody
-- was asked.
--
-- The connection a question is about is stored in typed columns rather than
-- recovered from a subject string, so nothing has to encode structure into an
-- identifier and then take it apart again.
ALTER TABLE tool_approvals RENAME TO tool_approvals_request_scope;
...
CREATE TABLE tool_approvals (
    ...
    -- Only a question about a connection can be remembered, because only that
    -- kind has a later connection for the memory to decide.
...
SELECT
    approval_id, revision, kind, exchange_id, access_id, plan_revision,
    plan_hash, subject_refs_json, subject_labels_json,
    -- A pre-migration network ask predates the typed target. There is exactly
    -- one such row shape and it is already resolved or about to be cancelled
    -- on recovery, so it is dropped rather than reconstructed from its
    -- subject string.
    '', 0,
    aggregate_key, request_count, waiter_count, state, decision,
    decision_scope, decision_reason, decision_idempotency_key,
    created_at_unix_ms, expires_at_unix_ms, resolved_at_unix_ms
FROM tool_approvals_request_scope
WHERE kind <> 'network_ask';

DROP TABLE tool_approvals_request_scope;
...
-- One target has one answer. Two rules matching the same connection would make
-- the answer depend on precedence a person never chose.
CREATE UNIQUE INDEX connection_rules_target
    ON connection_rules (match_kind, match_host, match_port)
    WHERE is_default = 0;
