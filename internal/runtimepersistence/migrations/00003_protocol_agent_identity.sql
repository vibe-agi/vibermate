-- +goose Up
-- Exact client protocol identifiers are structural evidence, not transcript
-- content. Persist them even when semantic recording is disabled; a later
-- client-local lookup may monotonically enrich the same Exchange row.
CREATE TABLE runtime_exchange_agent_identities_next(
...
INSERT INTO runtime_exchange_agent_identities_next(
...
SELECT exchange_id, client_kind, session_id, session_resumable,
       actor_id, actor_label, actor_type, actor_is_subagent,
       provider_response_id, provider_message_id,
       protocol_ids_json, attributes_json,
       evidence_source, confidence, observed_at_unix_ms
...
ALTER TABLE runtime_exchange_agent_identities_next
RENAME TO runtime_exchange_agent_identities;
...
-- +goose Down
CREATE TABLE runtime_exchange_agent_identities_previous(
  exchange_id TEXT PRIMARY KEY NOT NULL
  CHECK(length(CAST(exchange_id AS BLOB)) BETWEEN 1 AND 512),
  client_kind TEXT NOT NULL CHECK(client_kind IN('claude', 'codex')),
  session_id TEXT NOT NULL
...
INSERT INTO runtime_exchange_agent_identities_previous(
  exchange_id, client_kind, session_id, session_resumable,
  actor_id, actor_label, actor_type, actor_is_subagent,
  provider_response_id, provider_message_id,
  protocol_ids_json, attributes_json,
...
SELECT exchange_id, client_kind, session_id, session_resumable,
       ...
       evidence_source, confidence, observed_at_unix_ms
FROM runtime_exchange_agent_identities
WHERE evidence_source = 'client_local_state'
  AND provider_response_id <> '';
DROP TABLE runtime_exchange_agent_identities;
ALTER TABLE runtime_exchange_agent_identities_previous
RENAME TO runtime_exchange_agent_identities;
CREATE INDEX runtime_exchange_agent_identities_session
ON runtime_exchange_agent_identities(client_kind, session_id, exchange_id);
CREATE INDEX runtime_exchange_agent_identities_actor
