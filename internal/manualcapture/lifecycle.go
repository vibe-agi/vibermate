...
func (gate *lifecycleGate) begin(
	...
	finish := func() {
		once.Do(func() {
			stop()
			cancel(errors.New("ManualCapture operation finished"))
			gate.finish()
		})
	}
...
func (gate *lifecycleGate) finish() {
	...
	gate.active--
	if gate.active < 0 {
		panic("ManualCapture lifecycle active operation count became negative")
	}
...
func (gate *lifecycleGate) drain(ctx context.Context) error {
	...
	case <-drained:
		return nil
	case <-ctx.Done():
		return fmt.Errorf("drain ManualCapture operations: %w", ctx.Err())
	}
}
