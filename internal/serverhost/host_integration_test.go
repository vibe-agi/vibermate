...
func TestServerOwnerWebSessionCanManageManualCapture(t *testing.T) {
	...
	ownerReadURLs := []string{
		"http://" + host.Status().ListenAddress +
			"/api/v1/evidence/search?q=synthetic&limit=10",
		"http://" + host.Status().ListenAddress + "/api/v1/storage",
	}
	for _, ownerReadURL := range ownerReadURLs {
		for _, trial := range []struct {
			label      string
			credential string
			expected   int
			...
			searchRequest, requestErr := http.NewRequest(http.MethodGet, ownerReadURL, nil)
			if requestErr != nil {
				t.Fatal(requestErr)
			}
			if trial.credential != "" {
			...
			if searchResponse.StatusCode != trial.expected {
				t.Fatalf("%s owner read %s status=%d, want %d", trial.label, ownerReadURL, searchResponse.StatusCode, trial.expected)
			}
		}
	}
