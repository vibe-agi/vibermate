...
export async function applicationTreeLedger(root) {
  ...
  const visit = async (path, relativePath) => {
    ...
    if (metadata.isSymbolicLink()) {
      throw new Error("The macOS application must not contain symbolic links");
    }
...
async function inspectMachOPaths(appPath, ledger) {
  ...
  return Object.freeze(
    Object.fromEntries(
      macOSDistributionPolicy.executableNames.map((name) => [
        name,
        resolve(appPath, "Contents", "MacOS", name),
      ]),
    ),
  );
}
...
export async function inspectSignedMacOSApplicationAtPath(
  ...
  for (const [name, path] of Object.entries(executablePaths)) {
    ...
    inspectCodeSignature(path, expectedTeamID, {
      deep: false,
      expectedIdentifier: macOSDistributionPolicy.executableIdentifiers[name],
      label: name,
      requireRuntime: true,
    });
...
export async function inspectUnsignedMacOSDistributionCandidate() {
  ...
  for (const name of ["vibermate", "vibermated"]) {
    if (executableSHA256[name] !== buildProvenance.sidecarSHA256[name]) {
