...
async function fixture(t) {
  ...
  await writeFixtureFile(
    ...
    [
      "ui/desktop/dist/",
      "ui/desktop/src-tauri/binaries/",
      "ui/desktop/src-tauri/target/",
      "",
    ].join("\n"),
  );
  await writeFixtureFile(
    join(repositoryRoot, "ui/desktop/package.json"),
    `${JSON.stringify({ name: "@vibermate/desktop", version: "0.1.0" }, null, 2)}\n`,
  );
  await writeFixtureFile(
  ...
  const configurationPayloads = {
    ...
    "ui/desktop/pnpm-lock.yaml": "lockfileVersion: '9.0'\n",
    "ui/desktop/src-tauri/Cargo.toml":
      '[package]\nname = "vibermate-desktop"\nversion = "0.1.0"\n',
    "ui/desktop/src-tauri/Cargo.lock":
      "# This file is automatically @generated by Cargo.\nversion = 4\n",
    "ui/desktop/src-tauri/tauri.conf.json":
      `${JSON.stringify({ productName: "ViberMate", version: "0.1.0" }, null, 2)}\n`,
  };
  ...
  const mainPath = join(
    repositoryRoot,
    "ui/desktop/src-tauri/target/universal-apple-darwin/release/vibermate-desktop",
  );
  const launcherPath = join(
    repositoryRoot,
    "ui/desktop/src-tauri/binaries/vibermate-universal-apple-darwin",
  );
  const daemonPath = join(
    repositoryRoot,
    "ui/desktop/src-tauri/binaries/vibermated-universal-apple-darwin",
  );
  const buildManifestPath = join(
    repositoryRoot,
    "ui/desktop/src-tauri/binaries/vibermate-build-manifest.json",
  );
  const distPath = join(repositoryRoot, "ui/desktop/dist");
  ...
  await writeFixtureFile(join(distPath, "index.html"), "<!doctype html>\n");
  await writeFixtureFile(join(distPath, "assets/app.js"), "export {};\n");
...
async function externalInputRoot(value) {
  ...
  await writeFixtureFile(
    ...
    await readFile(join(value.distPath, "index.html")),
  );
  await writeFixtureFile(
    join(root, "dist/assets/app.js"),
    await readFile(join(value.distPath, "assets/app.js")),
  );
  return root;
}
...
test("source-traceability preparer (R0) consumes only the closed external build-input root", async (t) => {
  ...
  await rm(join(value.repositoryRoot, "ui/desktop/dist"), {
    recursive: true,
  });
  await rm(join(value.repositoryRoot, "ui/desktop/src-tauri/binaries"), {
    recursive: true,
  });
  await rm(join(value.repositoryRoot, "ui/desktop/src-tauri/target"), {
    recursive: true,
  });
  await prepareR0ReleaseEvidence(
...
test("source-traceability preparer (R0) binds build-manifest configuration and sidecar digests", async (t) => {
  ...
  await t.test("sidecar digest", async (t) => {
    const value = await fixture(t);
