...
export function validateEmbeddedBuildManifest(
  value,
  { expectedRevision },
) {
  requireExactKeys(
  ...
  requireExactKeys(
    ...
    "Embedded Desktop build manifest source",
  );
  requireExactKeys(
    value.profiles,
    ["desktop", "sidecars", "target"],
    "Embedded Desktop build manifest profiles",
  );
  requireExactKeys(
    value.toolchains,
    ["cargo", "go", "node", "pnpm", "rustc", "tauri"],
    "Embedded Desktop build manifest toolchains",
  );
  requireDigestObject(
    value.configurationSHA256,
    desktopBuildConfigurationNames,
    "Embedded Desktop build configuration digests",
  );
  requireDigestObject(
    value.sidecarSHA256,
    ["vibermate", "vibermated"],
    "Embedded Desktop sidecar digests",
  );
  if (
    value.schema !== "vibermate.desktop-build/v2" ||
    value.source.vcs !== "git" ||
    value.source.dirty !== false ||
    !fullGitRevisionPattern.test(value.source.revision ?? "") ||
    !fullGitRevisionPattern.test(expectedRevision ?? "") ||
    ...
    value.profiles.target !== macOSDistributionPolicy.target
  ) {
    throw new Error("Embedded Desktop build provenance is inconsistent");
  }
  for (const [name, toolchain] of Object.entries(value.toolchains)) {
    requireNonemptyString(
      toolchain,
      `Embedded Desktop ${name} toolchain`,
      16 << 10,
    );
  }
  return Object.freeze({
    ...
    sidecarSHA256: Object.freeze({ ...value.sidecarSHA256 }),
  });
}
...
export function validateMachOInventory(actualPaths) {
  ...
  const expected = macOSDistributionPolicy.executableNames
    .map((name) => `Contents/MacOS/${name}`)
    .sort();
  if (
    actual.length !== expected.length ||
    actual.some((path, index) => path !== expected[index])
  ) {
    throw new Error("The application must contain exactly three Mach-O files");
  }
...
export function validateSigningTransformationEvidence(value) {
  ...
  requireDigestObject(
    ...
    macOSDistributionPolicy.executableNames,
...
function validateNotaryTicketContents(value, archiveFilename) {
  ...
  for (const name of macOSDistributionPolicy.executableNames) {
    for (const architecture of macOSDistributionPolicy.architectures) {
      expectedTickets.add(
        `${archiveFilename}/${macOSDistributionPolicy.appBundleName}/Contents/MacOS/${name}\0${architecture}`,
      );
    }
  }
...
export function validateNotarizationEvidence(value) {
  ...
  if (
    ...
    value.notarization.ticketedCodeDirectories !== 6
  ) {
