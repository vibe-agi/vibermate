...
test("opens a request through its frozen Environment to the exact attempt", async ({ page }) => {
  ...
  await expect(page.getByText("New in this request · 1 new · 2 inherited", { exact: true })).toBeVisible();
  ...
  await expect(page.getByText("Provider reasoning state preserved, not displayed · 96 bytes", { exact: true })).toHaveCount(0);
  ...
  await page.getByRole("button", { name: "View full snapshot" }).click();
  await expect(page.getByText("Full client snapshot · total 3 · inherited 2", { exact: true })).toBeVisible();
  await expect(page.getByRole("button", { name: "Show this request" })).toBeVisible();
