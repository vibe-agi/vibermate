...
const configurationNames = flutterDesktopBuildConfigurationNames;
...
function flutterPackageVersion(payload) {
  let source;
  try {
    source = new TextDecoder("utf-8", { fatal: true }).decode(payload);
  } catch {
    throw new Error("Flutter pubspec.yaml is not valid UTF-8");
  }
  const matches = [...source.matchAll(/^version:\s*([^\s+#]+)(?:\+[^\s#]+)?\s*$/gmu)];
  if (
    matches.length !== 1 ||
    !semanticVersion.test(matches[0][1]) ||
    matches[0][1] === "0.0.0"
  ) {
    throw new Error(
      "Flutter package version must be one non-placeholder Semantic Version",
    );
  }
  return matches[0][1];
}
...
function validateDesktopBuildManifest(value, expectedRevision, commitTime) {
  try {
    validateFlutterDesktopBuildManifest(value, {
      expectedRevision,
    ...
    if (
      ...
      (value.source.revision !== expectedRevision || value.source.dirty !== false)
    ) {
      throw new Error(
        "Desktop build manifest does not bind the exact clean Git source",
        { cause: error },
      );
    }
    throw error;
  }
  if (value.source.commitTime !== commitTime) {
    throw new Error("Desktop build manifest does not bind the exact Git commit time");
  }
  return value;
}
...
export async function prepareR0ReleaseEvidence(
  ...
  const repositorySourcePaths = {
    package: "ui/flutter_app/pubspec.yaml",
  ...
  const inputSourcePaths = hasExternalInputRoot
    ...
    : {
        main: "ui/flutter_app/build/distribution/universal-apple-darwin/release/bundle/macos/ViberMate.app/Contents/MacOS/vibermate-desktop",
        vibermate: "ui/flutter_app/build/distribution/universal-apple-darwin/release/bundle/macos/ViberMate.app/Contents/MacOS/vibermate",
        vibermated: "ui/flutter_app/build/distribution/universal-apple-darwin/release/bundle/macos/ViberMate.app/Contents/MacOS/vibermated",
        desktopBuild: "ui/flutter_app/build/distribution/universal-apple-darwin/release/bundle/macos/ViberMate.app/Contents/Resources/vibermate-build-manifest.json",
        license: "LICENSE",
        dist: "ui/flutter_app/build/distribution/universal-apple-darwin/release/r0-dist",
      };
  if (hasExternalInputRoot) {
  ...
  const version = flutterPackageVersion(packageSource.payload);
  ...
  try {
    ...
    if (
      copied.vibermate.digest !== buildManifest.nestedCodeSHA256.vibermate ||
      copied.vibermated.digest !== buildManifest.nestedCodeSHA256.vibermated
    ) {
      throw new Error("actual sidecar bytes do not match Desktop nested-code provenance");
    }
    ...
    for (const [name, relativePath] of Object.entries({
      ...
      if (hashed.digest !== buildManifest.nestedCodeSHA256[name]) {
        throw new Error(
          `${name} bytes do not match Desktop nested-code provenance`,
        );
      }
    }
