...
export function signingCommandArguments(kind, identity, keychainPath, path) {
  ...
  if (
    ...
    macOSDistributionPolicy.codeObjects[kind] === undefined
  ) {
  ...
  return [
    ...
    "runtime",
    "--identifier",
    macOSDistributionPolicy.codeObjects[kind].identifier,
    path,
  ];
}
...
async function main() {
  ...
  for (const name of [
    ...
    const signingRelativePath =
      macOSDistributionPolicy.codeObjects[name].signingPath;
    const signingPath = resolve(
      unsigned.appPath,
      ...signingRelativePath.split("/"),
    );
    runTool(
      "/usr/bin/codesign",
