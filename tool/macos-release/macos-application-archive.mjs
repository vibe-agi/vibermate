...
function validateMode(mode, type) {
  if (type === "symlink") {
    if (mode !== 0o755) {
      throw new Error("The application archive contains an invalid link mode");
    }
    return;
  }
  if (
...
function validateEntryHeader(source, value, seenPortablePaths) {
  if (value.type === "directory") {
    ...
    if (
      !exactKeys(value, ["mode", "path", "size", "target", "type"]) ||
      value.size !== 0 ||
      ...
      throw new Error("The application archive symbolic-link header is not canonical");
    }
  } else {
  ...
  if (
    value.type === "symlink" &&
    macOSDistributionPolicy.allowedApplicationSymlinks[
      ledgerPathFromArchive(value.path)
    ] !== value.target
  ) {
    throw new Error("The application archive symbolic link is not admitted");
  }
  return value;
}
  ...
  try {
    ...
    while (position < archiveMetadata.size) {
      ...
      if (header.type === "directory") {
        ...
        await symlink(header.target, outputPath);
        const linkInfo = await lstat(outputPath);
        if (
          !linkInfo.isSymbolicLink() ||
          (await readlink(outputPath)) !== header.target
        ) {
          throw new Error("The application archive symbolic link was not restored");
        }
      }
      entryCount += 1;
