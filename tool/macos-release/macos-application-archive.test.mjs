...
test("closed App archive round-trips only the fixed Flutter framework links", async (context) => {
  const directory = await realpath(
    await mkdtemp(resolve(tmpdir(), "vibermate-archive-flutter-links-")),
  );
  context.after(() => rm(directory, { recursive: true, force: true }));
  const source = resolve(directory, "source", "ViberMate.app");
  const framework = resolve(source, "Contents", "Frameworks", "App.framework");
  const version = resolve(framework, "Versions", "A");
  await mkdir(resolve(version, "Resources"), { recursive: true, mode: 0o755 });
  await writeFile(resolve(version, "App"), "framework bytes\n", { mode: 0o755 });
  await symlink("A", resolve(framework, "Versions", "Current"));
  await symlink("Versions/Current/App", resolve(framework, "App"));
  await symlink("Versions/Current/Resources", resolve(framework, "Resources"));
  ...
  assert.equal(
    await readlink(resolve(restored, "Contents", "Frameworks", "App.framework", "App")),
...
test("closed App archive rejects links, extensions, and traversal records", async (context) => {
  ...
  context.after(() => rm(directory, { recursive: true, force: true }));
  const magic = Buffer.from("VIBERMATE-APP-ARCHIVE-V2\n", "ascii");
...
test("closed App archive rejects semantically equivalent reordered headers", async (context) => {
  ...
  await writeFile(
    archive,
    Buffer.concat([
      Buffer.from("VIBERMATE-APP-ARCHIVE-V2\n", "ascii"),
